...
def get_item(item_id):
    ...
    while try_count < MAX_ITEM_RETRIES:
        ...
        try:
            response = json.loads(
                requests.get(f"https://hacker-news.firebaseio.com/v0/item/{item_id}.json?print=pretty").text
            )
...
if __name__ == "__main__":
    ...
    stories = (
        HnStory.query.filter(HnStory.last_refreshed_at < now - timedelta(minutes=30))
        .filter(
            or_(
                ...
                and_(
                    HnStory.posted_at >= (now - timedelta(days=7)),
                    HnStory.last_refreshed_at <= (now - timedelta(days=1)),
                ),
                # then, one final time before getting archived
                and_(
                    HnStory.posted_at < (now - timedelta(days=14)),
